<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"20px 20px 20px","overflow":"hidden","color":"#93a2a9","background":"#e9ecf3","fontSize":"14px","position":"relative"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #ddd","padding":"20px 20px 0","margin":"0px 0 0","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"0px","margin":"0 0 10px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","background":"none","display":"flex","width":"auto","fontSize":"12px"}'>
					<el-button class="add" v-if="isAuth('xueshengpingjia','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('xueshengpingjia','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('xueshengpingjia','学生评价')" type="success" @click="chartDialog1()">
			...
			<div :style='{"border":"0px solid #bababa","padding":"0","margin":"0 0 0 0","borderRadius":"2px","background":"rgba(255,255,255,1)","width":"calc(100% - 0px)","float":"right"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('xueshengpingjia','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"inherit","color":"#3bb4f2","display":"inline-block"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('xueshengpingjia','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('xueshengpingjia','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="pingjiadengjiChart1" style="width:100%;height:600px;"></div>
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var pingjiadengjiChart1 = echarts.init(document.getElementById("pingjiadengjiChart1"),'macarons');
        this.$http({
            url: "xueshengpingjia/group/pingjiadengji",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].pingjiadengji);
                ...
                // 使用刚指定的配置项和数据显示图表。
                pingjiadengjiChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    pingjiadengjiChart1.resize();
                };
            }
        });
      })
	...
	xueshengpingjiastatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'xueshengpingjia/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
