...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="企业账号" prop="qiyezhanghao">
            <el-select  @change="qiyezhanghaoChange" v-model="ruleForm.qiyezhanghao" placeholder="请选择企业账号">
              <el-option
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="评价等级" prop="pingjiadengji">
            <el-select v-model="ruleForm.pingjiadengji" placeholder="请选择评价等级"  >
              <el-option
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/qiye/qiyezhanghao', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.qiyezhanghaoOptions = res.data.data;
          }
        });
      ...
      // 下二随
      qiyezhanghaoChange () {
        this.$http.get('follow/qiye/qiyezhanghao?columnValue=' + this.ruleForm.qiyezhanghao, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            if(res.data.data.fuzeren){
              this.ruleForm.fuzeren = res.data.data.fuzeren
            }
            if(res.data.data.qiyemingcheng){
              this.ruleForm.qiyemingcheng = res.data.data.qiyemingcheng
            }
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`xueshengpingjia/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('xueshengpingjia/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`xueshengpingjia/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
